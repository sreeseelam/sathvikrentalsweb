...
== Linking persons with their managers

The last bit of modeling security is to associate persons with a manager. In this domain, an person can have one manager while a manager can have multiple persons:

[source,java]
----
include::src/main/java/com/greglturnquist/payroll/Person.java[tag=code]
----
...
== Securing persons to their managers

Spring Security supports a multitude of options when it comes to defining security policies. In this section, you want to restrict things such that ONLY managers can view person payroll data, and that saving, updating, and deleting operations are confined to the person's manager. In other words, any manager can log in and view the data, but only a given person's manager can make any changes.

[source,java]
----
include::src/main/java/com/greglturnquist/payroll/PersonRepository.java[tag=code]
----

`@PreAuthorize` at the top of the interface restricts access to people with *ROLE_MANAGER*.

On `save()`, either the person's manager is null (initial creation of a new person when no manager has been assigned), or the person's manager's name matches the currently authenticated user's name. Here you are using http://docs.spring.io/spring-security/site/docs/current/reference/htmlsingle/#el-access[Spring Security's SpEL expressions] to define access. It comes with a handy "?." property navigator to handle null checks. It's also important to note using the `@Param(...)` on the arguments to link HTTP operations with the methods.

On `delete()`, the method either has access to the person, or in the event it only has an id, then it must find the *personRepository* in the application context, perform a `findOne(id)`, and then check the manager against the currently authenticated user.
...
`@RepositoryEventHandler(Person.class)` flags this event handler as only applied to `Person` objects. The `@HandleBeforeCreate` annotation gives you a chance to alter the incoming `Person` record before it gets written to the database.
...
Loading managers and linking persons to these managers is rather straight forward:
...
The one wrinkle is that Spring Security is active with access rules in full force when this loader runs. Thus to save person data, you must use Spring Security's `setAuthentication()` API to authenticate this loader with the proper name and role. At the end, the security context is cleared out.
...
----
$ curl -v -u greg:turnquist localhost:8080/api/persons/1
*   Trying ::1...
* Connected to localhost (::1) port 8080 (#0)
* Server auth using Basic with user 'greg'
> GET /api/persons/1 HTTP/1.1
> Host: localhost:8080
...
With all these mods in the backend, you can now shift to updating things in the frontend. First of all, show an person's manager inside the `<Person />` React component:

[source,javascript,indent=0]
----
include::src/main/js/app.js[tag=person]
----

This merely adds a column for `this.props.person.entity.manager.name`.
...
You can see the newly added manager column. Go through a couple pages until you find persons owned by *oliver*.
...
If you try *Delete*, it should fail with a similar message. Create a new person, and it should be assigned to you.

== Review

In this section:

* You defined the model of manager and linked it to an person via a 1-to-many relationship.
...
* You wrote another Spring Data REST event handler to trap creation events before they happen so they current user could be assigned as the person's manager.
* You updated the UI to show an person's manager and also display error pop-ups when unauthorized actions are taken.
...
Managers have access to person data. Should persons have access? If you were to add more details like phone numbers and addresses, how would you model it? How would you grant persons access to the system so they could update those specific fields? Are there more hypermedia controls that would be handy to put on the page?
